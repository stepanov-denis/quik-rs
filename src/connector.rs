//! Модуль для установления/разрыва связи с терминалом QUIK.
pub mod terminal {
    ...
    pub fn load_dll(path: &str) -> Result<Library, libloading::Error> {
        unsafe {
    ...
    pub fn connect(lib: &Library) -> bool {
        // Определяем тип функции
        unsafe {
            // Найдем функцию TRANS2QUIK_CONNECT в библиотеке
            let connect: Symbol<unsafe extern "C" fn(*const c_char, *mut c_long, *mut c_char, c_ulong) -> c_long> = lib.get(b"TRANS2QUIK_CONNECT\0").expect("Could not find function");
            ...
            match result {
                0 => info!("TRANS2QUIK_SUCCESS - соединение установлено успешно"),
                2 => {
